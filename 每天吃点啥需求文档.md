...
|------|------|------|------|
| id | BigAutoField | Primary Key | 主键 |
| device_id | CharField(64) | Not Null, Unique | 设备唯一标识（用于设备级用户识别） |
| preferences | JSONField | Default=dict | 口味偏好设置，格式：`{"allergies": [], "dislikes": [], "favorites": []}` |
| created_at | DateTimeField | Auto Now Add | 创建时间 |
| updated_at | DateTimeField | Auto Now | 更新时间 |
...
**备注：** MVP 阶段不使用此表，收藏功能通过前端 AsyncStorage 本地存储。
